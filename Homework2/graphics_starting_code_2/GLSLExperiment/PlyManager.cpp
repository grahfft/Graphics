...
Ply PlyManager::GetCurrentPly() 
{
	...
	cout << "Getting polygon " << currentPly.getFilename() << endl;
...
Ply PlyManager::GetNextPly()
{
	...
	cout << "Getting Next Polyon current index: " << this->currentPly << endl;
...
Ply PlyManager::GetPreviousPly()
{
	...
	cout << "Getting Previous Polyon current index: " << this->currentPly << endl;
...
void PlyManager::createPly(string filename)
{
	...
	if (polygon->LoadGeometry())
	{
		...
		cout << "ERROR!!!! Failed to load Geometry: " << filename << endl;
	}
}
